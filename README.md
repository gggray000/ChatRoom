...
<h3>v0.8.8</h3>
Resolved race condition for ChatBot's `SystemMessage`.
<h3>v0.8.7</h3>
...
2. Now each URL stands for a unique `ChatRoom`. Refactored controllers and WebSocket code, added new directory `/room` to handle this.<br>
...
There's a bug in his video, `message.type` should be `<code>`message.messageType`</code>` in `main.js`.
